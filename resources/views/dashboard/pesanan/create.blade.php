                    ...
                    @foreach ($users as $user)
                        @if ($user->rule == 'Customer')
                            ...
                            <x-input-pesanan class="row mb-sm-3 mb-3"
                                ...
                                type="text"
                                name="alamat"
                                :value="$user->jalan . ' No. ' . $user->nomor . ' Kec. ' . $user->kecamatan->nama_kecamatan .', Kab. '. $user->kabupaten->nama_kabupaten"
                                :readonly=true
                            />
                        @endif  
                        ...
                        <div class="col-sm-9 col-12 mt-1">
                            <select class="form-select" id="produk" name="id_produk">
                                @foreach ($produk as $prd)
                                        <option value="{{ old('id_produk', $prd->id) }}">{{ $prd->nama_produk }}</option>   
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9 col-12 mt-1">
                            <select class="form-select" id="statusPembayaran" name="id_status_pembayaran">
                                @foreach ($pembayaran as $pmb)
                                    <option value="{{ old('id_status_pembayaran', $pmb->id) }}">{{ $pmb->status_pembayaran }}</option>   
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9 col-12 mt-1">
                            <select class="form-select" id="statusPesanan" name="id_metode_pembayaran">
                                @foreach ($metode as $mtd)
                                    <option value="{{ old('id_metode_pembayaran', $mtd->id) }}">{{ $mtd->metode_pembayaran . ' (' . $mtd->no_rekening . ')'}}</option>   
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9 col-12 mt-1">
                            <select class="form-select" id="pengiriman" name="id_pengiriman">
                                @foreach ($ekspedisi as $eks)
                                    <option value="{{ old('id_pengiriman', $eks->id) }}" data-eks="{{ $eks->id }}">{{ $eks->nama_pengiriman }}</option>   
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="row mb-sm-3 mb-3">
                            ...
                            <div class="col-sm-9 col-12 mt-1">
                                <div class="input-group">
                                  <textarea class="form-control" id="deskripsi" aria-label="With textarea" name="deskripsi">{{ old('deskripsi') }}</textarea>  
                                </div>                  
                            </div>
                        </div>
                        @endif
